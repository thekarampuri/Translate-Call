...
public class WalkieTalkieService extends VoiceTranslationService {
    ...
    public void initialize() {
                    ...
                    if (!WalkieTalkieService.super.executeCommand(command, message.getData())) {
                        switch (command) {
                            case CHANGE_FIRST_LANGUAGE:
                                CustomLocale newFirstLanguage = (CustomLocale) message.getData()
                            ...
                            case CHANGE_SECOND_LANGUAGE:
                                CustomLocale newSecondLanguage = (CustomLocale) message.getData()
                                        .getSerializable("language");
                            ...
                            case RECEIVE_TEXT:
                                ...
                                if (text != null) {
                                    ...
                                    translator.detectLanguage(new NeuralNetworkApiResult(text), true,
                                            new Translator.DetectLanguageListener() {
                                                @Override
                                                public void onDetectedText(final NeuralNetworkApiResult result) {
                                                    // here the result returns with the same language as the text sent
                                                    // from the Fragment editText
                                                    if (result.getLanguage().equalsLanguage(firstLanguage)) {
                                                        translator.translate(result.getText(), firstLanguage,
                                                                secondLanguage, TRANSLATOR_BEAM_SIZE, false,
                                                                firstResultTranslateListener);
                                                    } else {
                                                        translator.translate(result.getText(), secondLanguage,
                                                                firstLanguage, TRANSLATOR_BEAM_SIZE, false,
                                                                secondResultTranslateListener);
                                                    }
                                                }
        ...
        mVoiceCallback = new Recorder.SimpleCallback() {
            ...
            public void onVoice(@NonNull float[] data, int size) {
                ...
                if (speechRecognizer == null) {
                    Log.e("WalkieTalkieService", "speechRecognizer is null, cannot recognize voice");
                    // Restart the mic since we can't process the voice
                    startVoiceRecorder();
                    notifyMicActivated();
                    return;
                }
        ...
        speechRecognizerCallback = new RecognizerMultiListener() {
            @Override
            public void onSpeechRecognizedResult(String text1, String languageCode1, double confidenceScore1,
        ...
        speechRecognizerSingleCallback = new RecognizerListener() {
            ...
            public void onSpeechRecognizedResult(String text, String languageCode, double confidenceScore,
                    boolean isFinal) {
                if (manualRecognizingFirstLanguage) {
                    translate(text, firstLanguage, secondLanguage, TRANSLATOR_BEAM_SIZE, false,
                            firstResultTranslateListener);
                } else if (manualRecognizingSecondLanguage) {
                    translate(text, secondLanguage, firstLanguage, TRANSLATOR_BEAM_SIZE, false,
                            secondResultTranslateListener);
                }
                manualRecognizingFirstLanguage = false;
                manualRecognizingSecondLanguage = false;
        ...
        firstResultTranslateListener = new Translator.TranslateListener() {
            @Override
            public void onTranslatedText(String textToTranslate, String text, long resultID, boolean isFinal,
                ...
                ((Global) getApplication()).getTTSLanguages(true, new Global.GetLocalesListListener() {
                    ...
                    public void onSuccess(ArrayList<CustomLocale> ttsLanguages) {
                        if (isFinal && CustomLocale.containsLanguage(ttsLanguages, languageOfText)) { // check if the
                                                                                                      // text can be
                                                                                                      // speak
                            speak(text, languageOfText);
                        }
                        GuiMessage message = new GuiMessage(
                                new Message(textToTranslate, WalkieTalkieService.this, text), resultID, true, isFinal);
                        ...
                        if (isFinal && (tts == null || !CustomLocale.containsLanguage(ttsLanguages, languageOfText)
                                || !tts.isActive() || isAudioMute)) {
                            startVoiceRecorder();
                            notifyMicActivated();
                        }
                    }
            ...
            public void onTranslatedText(String textToTranslate, String text, long resultID, boolean isFinal,
                ...
                ((Global) getApplication()).getTTSLanguages(true, new Global.GetLocalesListListener() {
                    ...
                    public void onSuccess(ArrayList<CustomLocale> ttsLanguages) {
                        ...
                        GuiMessage message = new GuiMessage(
                                new Message(textToTranslate, WalkieTalkieService.this, text), resultID, false, isFinal);
                        WalkieTalkieService.super.notifyMessage(message);
                        // we save every new message in the exchanged messages so that the fragment can
                        // restore them
                        WalkieTalkieService.super.addOrUpdateMessage(message);
    ...
    private void compareResults(NeuralNetworkApiResult firstResult, NeuralNetworkApiResult secondResult) {
        translator.detectLanguage(firstResult, secondResult, false, new Translator.DetectMultiLanguageListener() {
            @Override
            public void onDetectedText(NeuralNetworkApiResult firstResult, NeuralNetworkApiResult secondResult,
                    int message) {
                if (message == ErrorCodes.BOTH_RESULTS_SUCCESS) { // if both results languages were found
                    if (firstResult.getText().equals(Recognizer.UNDEFINED_TEXT)
                            && !secondResult.getText().equals(Recognizer.UNDEFINED_TEXT)) {
                        translate(secondResult.getText(), secondLanguage, firstLanguage, TRANSLATOR_BEAM_SIZE, false,
                                secondResultTranslateListener);
                        return;
                    }
                    if (secondResult.getText().equals(Recognizer.UNDEFINED_TEXT)
                            && !firstResult.getText().equals(Recognizer.UNDEFINED_TEXT)) {
                        translate(firstResult.getText(), firstLanguage, secondLanguage, TRANSLATOR_BEAM_SIZE, false,
                                firstResultTranslateListener);
                        return;
                    }

                    if (firstResult.getLanguage().equalsLanguage(firstLanguage)) {
                        if (secondResult.getLanguage().equalsLanguage(secondLanguage)) { // if both have recognized
                        ...
                        if (secondResult.getLanguage().equalsLanguage(secondLanguage)) { // if only the second result
                                                                                         // language is recognized
                            translate(secondResult.getText(), secondLanguage, firstLanguage, TRANSLATOR_BEAM_SIZE,
                                    false, secondResultTranslateListener);
    ...
    private void translate(final String textToTranslate, final CustomLocale languageInput,
            final CustomLocale languageOutput, int beamSize, boolean saveResults,
            final Translator.TranslateListener responseListener) {
        if (!isMetaText(textToTranslate)) {
            translator.translate(textToTranslate, languageInput, languageOutput, beamSize, saveResults,
                    responseListener);
        } else {
            // we restart the mic here
            startVoiceRecorder();
